<?php require 'header.php'  ?>

<main class="max_width main" id="home">
  <section class="main_text">
    <h3 class="welcome">Welcome</h3>
    <h3 class="main_heading">We make the best burger in town</h3>
    <p>
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Eligendi, tenetur consequuntur
      officiis magnam ipsam minima!
    </p>
  </section>
  <section class="main_image_section">
    <div class="main_image_container">
      <img src="./Images/main-burger.png" alt="Burger Image" />
    </div>
  </section>
</main>

<section class="order_and_delivery_section">
  <div class="order_and_delivery_container max_width">
    <div class="delivery_container">
      <h3 class="title">Reservation</h3>
      <h3 class="delivery_heading">Choose what you want and select time</h3>
      <p>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Eligendi, tenetur consequuntur
        officiis magnam ipsam minima!
      </p>
      <button>Book a Table</button>
    </div>
    <div class="reward_container">
      <h3 class="title">Reward</h3>
      <h3 class="reward_heading">Earn points every time you order online</h3>
      <p>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Eligendi, tenetur consequuntur
        officiis magnam ipsam minima!
      </p>
      <button>Learn More</button>
    </div>
  </div>
</section>

<section class="about_us max_width" id="about_us">
  <div>
    <h3 class="about_us_heading">More About us</h3>
    <p class="about_us_text">
      Seasonality and support of our local community are central to our philosophy at Berg. We’re passionate about cooking with local, seasonal ingredients and love how they can inspire new dishes. Not only do fresh, in-season ingredients enhance flavour, they are also great for you and their consumption minimises impact on the environment.
    </p>
  </div>

  <section class="restaurant_info">
    <div class="restaurant_address">
      <div>
        <img src="./Images/location.png" alt="Location">
      </div>
      <h4>Address</h4>
      <p>
        103 North Loundoun Street., Winchester, VA
      </p>
    </div>
    <div class="restaurant_open_time">
      <div>
        <img src="./Images/clock.png" alt="Clock">
      </div>
      <h4>We are open</h4>
      <p>
        <span><b>Mon – Fri:</b> 9am-10pm</span>
        <span><b>Sat:</b> 10pm-4am</span>
        <span><b>Sun:</b> 10am-7pm</span>
      </p>
    </div>
    <div class="restaurant_contact">
      <div>
        <img src="./Images/telephone.png" alt="Telephone">
      </div>
      <h4>Reservation</h4>
      <p>
        <span>+63 9202740366</span>
        <span>help@dinery.com</span>
      </p>
    </div>
  </section>

</section>


<?php require 'footer.php' ?>